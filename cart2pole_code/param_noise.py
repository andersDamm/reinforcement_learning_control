...
seed_env=36
env = dmc2gym.make(domain_name='cartpole', task_name='two_poles',seed=seed_env)
env.seed(seed_env)
...
class MuNet(nn.Module):
    ...
    def __init__(self, n, o, learning_rate):
        ...
        self.l2norm = nn.LayerNorm(400)
        self.l3norm = nn.LayerNorm(300)
...
def trainloop():
    ...
    for i in range(num_episodes):
        ...
        # adaptive scaling of parameter noise
        if i > 1:
            ss,_,_,_,_  = memory.sample(batch_size)
            # DDPG distance measure
            d = torch.sqrt(torch.mean(torch.pow(mu(ss) - mu_tilde(ss),2)))
            if d < sigma:
                sigma = sigma*alpha
            else:
                sigma = sigma/alpha
         
        # make net with parameter noise
        mu_tilde = copy.deepcopy(mu)
        param_noise(mu_tilde,sigma)
                
        while not done:

            a = mu_tilde(torch.from_numpy(s).float()) 
